...
// [26Jan2018]Quick has following issues with me:
...
//[27Jan2018] Issue is able to be resolved. XCTest code still keep there for reference purpose.
...
class PSIServiceSpec: QuickSpec {
    override func spec() {
        ...
        describe("PSIService"){
            ...
            it("should get nil when pass in invalid psiResponse"){
                let invalidPsiResponse = validPsiResponse
                invalidPsiResponse?.items = nil
                let actualRegions = psiService.fillPsiIndexInRegions(psiResponse: validPsiResponse!)

                expect(actualRegions).to(beNil())

            }

            it("should not get nil only when some psiIndex is missing"){
                let psiResponse = validPsiResponse!
                psiResponse.items![0].readings?.psi_twenty_four_hourly?.central = nil
                let actualRegions = psiService.fillPsiIndexInRegions(psiResponse: psiResponse)
